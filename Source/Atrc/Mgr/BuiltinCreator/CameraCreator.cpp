...
Camera *PinholeCameraCreator::Create(const ConfigGroup &group, Context &context, Arena &arena) const
{
    AGZ_HIERARCHY_TRY
    {
        ...
        if(sensorRectSize.x <= 0 || sensorRectSize.y <= 0)
            throw std::runtime_error("Invalid sensor rect size value");
        
        auto sensorDistance = group["sensorDistance"].Parse<Real>();

        if(sensorDistance <= 0)
            throw std::runtime_error("Invalid sensorDistance value");
        ...
        if(up == Vec3(Real(0)))
            throw std::runtime_error("Up vector must be non-zero");
