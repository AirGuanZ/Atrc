...
namespace bcd
{
			...
			float i_thresholdStDevFactor)
	{
		...
		for(int line = 0; line < height; ++line)
			...
			{
				int patchCenterLine = (line < patchRadius ? patchRadius :
						(line > height - 1 - patchRadius ? height - 1 - patchRadius : line));
				int patchCenterColumn = (column < patchRadius ? patchRadius :
			...
			const vector<float>& i_rData)
	{
		...
		o_rStandardDeviation = std::sqrt(total / (nbOfElements - 1));
	}
