...
public:
    ...
    explicit EntityBVH(CustomedFlag customed_flag = DEFAULT_CUSTOMED_FLAG)
    {
    ...
    static std::string description()
    {
        return R"___(
bvh [Aggregate]
    max_leaf_size [int] (optional; defaultly set to 5) maximum leaf size
)___";
    }
    ...
    void initialize(const Config &params, obj::ObjectInitContext &init_ctx) override
    {
        ...
        int max_leaf_size = params.child_int_or("max_leaf_size", max_leaf_size_);

        initialize(max_leaf_size);
    ...
    void build(const Entity *const*entities, size_t entity_count) override
    {
        ...
        prims_.reserve(entity_count);
        for(size_t i = 0; i < entity_count; ++i)
            records[i] = { entities[i], entities[i]->world_bound() };
...
Aggregate *create_entity_bvh(
    ...
    Arena &arena)
{
    auto ret = arena.create<EntityBVH>(customed_flag);
    ret->initialize(max_leaf_size);
    return ret;
}

AGZT_IMPLEMENTATION(Aggregate, EntityBVH, "bvh")
