...
{
    const PathTracingIntegrator *integrator_ = nullptr;
    int worker_count_ = 0;
...
public:
    ...
    explicit PathTracer(CustomedFlag customed_flag = DEFAULT_CUSTOMED_FLAG)
    {
...
pt [Renderer]
    ...
    void initialize(const PathTracingIntegrator *integrator, int worker_count, int task_grid_size, const Sampler *sampler_prototype)
    {
        AGZ_HIERARCHY_TRY
        ...
        task_grid_size_ = task_grid_size;
        if(task_grid_size_ <= 0)
            throw ObjectConstructionException("invalid task grid size value: " + std::to_string(task_grid_size_));

        sampler_prototype_ = sampler_prototype;
...
Renderer *create_path_tracer(
    ...
    Arena &arena)
{
    auto ret = arena.create<PathTracer>(customed_flag);
    ret->initialize(integrator, worker_count, task_grid_size, sampler_prototype);
    return ret;
}
