...
public:
    ...
    explicit Disk(CustomedFlag customed_flag = DEFAULT_CUSTOMED_FLAG)
    {
...
disk [Geometry]
    ...
    void initialize(const Config &params, obj::ObjectInitContext&) override
    {
        ...
        initialize(radius, local_to_world);
...
Geometry *create_disk(
    real radius, const Transform3 &transform,
    obj::Object::CustomedFlag customed_flag,
    Arena &arena)
{
    auto ret = arena.create<Disk>(customed_flag);
    ret->initialize(radius, transform);
    return ret;
}

AGZT_IMPLEMENTATION(Geometry, Disk, "disk")
