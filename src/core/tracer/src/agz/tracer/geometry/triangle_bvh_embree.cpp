...
public:
    ...
    explicit TriangleBVHEmbree(CustomedFlag customed_flag = DEFAULT_CUSTOMED_FLAG)
    {
...
triangle_bvh(_embree) [Geometry]
    ...
    void initialize(const Config &params, obj::ObjectInitContext &init_ctx) override
    {
        ...
        initialize(filename, local_to_world, *init_ctx.arena);

        AGZ_HIERARCHY_WRAP("in initializing triangle_bvh_embree_ws geometry object")
    }

    void initialize(const std::string &filename, const Transform3 &local_to_world, Arena &arena)
    {
        AGZ_HIERARCHY_TRY

        auto mesh = load(filename, local_to_world, arena);
        assert(mesh);
        untransformed_ = mesh;
...
Geometry *create_triangle_bvh_embree(
    ...
    Arena &arena)
{
    auto ret = arena.create<TriangleBVHEmbree>(customed_flag);
    ret->initialize(filename, local_to_world, arena);
    return ret;
}
...
Geometry *create_triangle_bvh(
    ...
    Arena &arena)
{
    return create_triangle_bvh_embree(filename, local_to_world, customed_flag, arena);
}
