...
class IdealDiffuse : public Material
{
    ...
    const NormalMapper *normal_mapper_;

public:

    explicit IdealDiffuse(CustomedFlag customed_flag = DEFAULT_CUSTOMED_FLAG)
    {
...
ideal_diffuse [Material]
    ...
    void initialize(const Config &params, obj::ObjectInitContext &context) override
    {
        ...
        auto normal_mapper = context.arena->create<NormalMapper>();
        normal_mapper->initialize(params, context);
        normal_mapper_ = normal_mapper;

        AGZ_HIERARCHY_WRAP("in initializing ideal diffuse material")
    }

    void initialize(const Texture *albedo, const NormalMapper *normal_mapper)
    {
        albedo_ = albedo;
        normal_mapper_ = normal_mapper;
    }
...
Material *create_ideal_diffuse(
    ...
    Arena &arena)
{
    auto ret = arena.create<IdealDiffuse>(customed_flag);
    ret->initialize(albedo, normal_mapper);
    return ret;
}
