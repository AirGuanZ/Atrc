...
public:
    ...
    explicit FrostedGlass(CustomedFlag customed_flag = DEFAULT_CUSTOMED_FLAG)
    {
...
frosted_glass [Material]
    ...
    void initialize(const Texture *color_map, const Texture *roughness, const Fresnel *fresnel)
    {
...
Material *create_frosted_glass(
    ...
    Arena &arena)
{
    auto ret = arena.create<FrostedGlass>(customed_flag);
    ret->initialize(color_map, roughness, fresnel);
    return ret;
}
