...
public:
    ...
    explicit Disney(CustomedFlag customed_flag = DEFAULT_CUSTOMED_FLAG)
    {
...
disney [Material]
    ...
    void initialize(const Config &params, obj::ObjectInitContext &init_ctx) override
    {
        ...
        auto normal_mapper = init_ctx.arena->create<NormalMapper>();
        normal_mapper->initialize(params, init_ctx);
        ...
        const NormalMapper *normal_mapper)
    {
        ...
        scatter_distance_       = scatter_distance;
...
Material *create_disney(
    ...
    Arena &arena)
{
    auto ret = arena.create<Disney>(customed_flag);
    ret->initialize(
        base_color,
        metallic,
        roughness,
