...
void Camera::update_camera_params()
{
    ...
    float sensor_height = sensor_width / aspect_;

    float cos_theta = std::cos(vert_rad_);
    ...
    float x_len = sensor_width;
    float y_len = sensor_height;
    vec3 x_ori = x_len * x_dir;
    vec3 y_ori = y_len * y_dir;
