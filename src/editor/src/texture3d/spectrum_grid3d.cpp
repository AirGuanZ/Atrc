...
SpectrumGrid3DWidget::SpectrumGrid3DWidget(const InitData &clone_state)
{
    init_ui(clone_state);
...
ResourceWidget<tracer::Texture3D> *SpectrumGrid3DWidget::clone()
{
    InitData clone_state;
    clone_state.img_data = img_data_;
    clone_state.adv      = adv_widget_->clone();
    return new SpectrumGrid3DWidget(clone_state);
}

void SpectrumGrid3DWidget::update_tracer_object_impl()
{
    do_update_tracer_object();
}

void SpectrumGrid3DWidget::do_update_tracer_object()
{
    ...
    tracer_object_ = create_spectrum_grid_point3d(common_params, img_data_);
}

void SpectrumGrid3DWidget::init_ui(const InitData &clone_state)
{
...
void SpectrumGrid3DWidget::save_asset(AssetSaver &saver)
{
    ...
    const uint32_t byte_size = img_data_->size().product() * sizeof(Spectrum);
    saver.write_raw(img_data_->raw_data(), byte_size);

    adv_widget_->save_asset(saver);
}

void SpectrumGrid3DWidget::load_asset(AssetLoader &loader)
{
    ...
    const uint32_t byte_size = width * height * depth * sizeof(Spectrum);
    ...
    JSONExportContext &ctx) const
{
    ...
    grp->insert_str("type", "spectrum_grid");
...
void SpectrumGrid3DWidget::browse_filename()
{
    try
    {
        bool ok = false;
