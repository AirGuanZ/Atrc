...
std::unique_ptr<ResourceThumbnailProvider> GrayGrid3DWidget::get_thumbnail(int width, int height) const
{
    // TODO
...
void GrayGrid3DWidget::load_asset(AssetLoader &loader)
{
    ...
    img_data_ = std::make_shared<Image3D<real>>(
...
void GrayGrid3DWidget::browse_filename()
{
    try
    {
        ...
        img_data_ = std::make_shared<Image3D<real>>(std::move(data));
    }
    catch(...)
    {
...
ResourceWidget<tracer::Texture3D> *GrayGrid3DWidgetCreator::create_widget(ObjectContext &obj_ctx) const
{
