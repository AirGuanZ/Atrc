...
PerPixelRenderer::PerPixelRenderer(
    int worker_count, int task_grid_size, int framebuffer_width, int framebuffer_height,
    int fast_resolution, int fast_task_grid_size, std::shared_ptr<const tracer::Scene> scene)
    : framebuffer_(framebuffer_width, framebuffer_height, task_grid_size)
{
...
Image2D<math::color3b> PerPixelRenderer::do_fast_rendering()
{
    ...
    auto sampler_prototype = create_native_sampler(1, 42, false);
    ...
    Image2D<math::color3b> target(framebuffer_height_, framebuffer_width_);
    thread::parallel_forrange(0, framebuffer_height_, [&](int y)
    {
        const int small_y = math::clamp<int>(
            static_cast<int>(real(y) / framebuffer_height_ * small_height), 0, small_height - 1);
        const int source_y = small_height - 1 - small_y;
        for(int x = 0; x < framebuffer_width_; ++x)
        {
            const int source_x = math::clamp<int>(
                static_cast<int>(real(x) / framebuffer_width_ * small_width), 0, small_width - 1);
            target(y, x) = small_target(source_y, source_x);
        }
    }, worker_count_);
