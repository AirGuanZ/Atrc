...
std::unique_ptr<Renderer> RendererPanel::create_renderer(std::shared_ptr<tracer::Scene> scene, const Vec2i &framebuffer_size) const
{
    if(renderer_widget_)
        return renderer_widget_->create_renderer(std::move(scene), framebuffer_size);
    return nullptr;
}
