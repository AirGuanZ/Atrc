...
namespace
{
    ...
    public:
        ...
        {
            std::shared_ptr<const texture::texture3d_t<Spectrum>> data;
            if(auto *str_child = params.find_child_value("ascii_filename"))
            {
                data = std::make_unique<Image3D<Spectrum>>(
