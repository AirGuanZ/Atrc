...
namespace microfacet
{
    ...
    Vec3 sample_gtr2(real alpha, const Sample2 &sample) noexcept
    {
        ...
        const real cos_theta = std::sqrt((1 - sample.v) / (1 + (sqr(alpha) - 1) * sample.v));
    ...
    Vec3 sample_anisotropic_gtr2(real ax, real ay, const Sample2 &sample) noexcept
    {
        ...
        const real A = compute_a(sin_phi_h, cos_phi_h, ax, ay);
        const real cos_theta_h = std::sqrt(A * (1 - sample.v) / ((1 - A) * sample.v + A));
        const real sin_theta_h = std::sqrt((std::max<real>)(0, 1 - sqr(cos_theta_h)));
    ...
    {
        const real phi = 2 * PI_r * sample.u;
        const real cos_theta = std::sqrt((std::pow(alpha, 2 - 2 * sample.v) - 1) / (sqr(alpha) - 1));
        const real sin_theta = local_angle::cos_2_sin(cos_theta);
        return Vec3(sin_theta * std::cos(phi), sin_theta * std::sin(phi), cos_theta).normalize();
    }
