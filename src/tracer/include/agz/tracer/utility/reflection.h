...
{
    real cos_theta_i = dot(w, n);
    real sin2_theta_t = math::sqr(eta) * (1 - math::sqr(cos_theta_i));
    real cos2_theta_t = 1 - sin2_theta_t;
    if(cos2_theta_t <= 0)
        return std::nullopt;
    real cos_theta_t = std::sqrt(cos2_theta_t);
...
inline real dielectric_fresnel(real eta_i, real eta_o, real cos_theta_i) noexcept
{
    ...
    real sin_theta_t = eta_o / eta_i * sin_theta_i;

    if(sin_theta_t >= 1)
        return 1;

    real cos_theta_t = std::sqrt((std::max)(real(0), 1 - sin_theta_t * sin_theta_t));
    real para = (eta_i * cos_theta_i - eta_o * cos_theta_t) / (eta_i * cos_theta_i + eta_o * cos_theta_t);
    real perp = (eta_o * cos_theta_i - eta_i * cos_theta_t) / (eta_o * cos_theta_i + eta_i * cos_theta_t);
